...
CREATE TABLE user_access_tokens(id INTEGER PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id),purpose TEXT NOT NULL,token_hash TEXT NOT NULL UNIQUE,expires_at TEXT NOT NULL,used_at TEXT,created_by INTEGER REFERENCES users(id),created_at TEXT DEFAULT CURRENT_TIMESTAMP);`,
  ...
  {
    version: 10,
    name: "shared_employee_training_log",
    sqlite: `CREATE TABLE employee_training(id INTEGER PRIMARY KEY,venue_id INTEGER NOT NULL REFERENCES venues(id),employee_name TEXT NOT NULL,job_title TEXT,training_date TEXT,allergens_in_house INTEGER NOT NULL DEFAULT 0,allergens_course INTEGER NOT NULL DEFAULT 0,food_hygiene_2 INTEGER NOT NULL DEFAULT 0,food_hygiene_3 INTEGER NOT NULL DEFAULT 0,fire_alarm INTEGER NOT NULL DEFAULT 0,fire_evacuation INTEGER NOT NULL DEFAULT 0,fire_extinguisher INTEGER NOT NULL DEFAULT 0,fire_exits INTEGER NOT NULL DEFAULT 0,emergency_procedures INTEGER NOT NULL DEFAULT 0,notes TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP,created_by INTEGER,updated_at TEXT DEFAULT CURRENT_TIMESTAMP,updated_by INTEGER); CREATE INDEX idx_employee_training_venue ON employee_training(venue_id,employee_name);`,
    azure: `IF OBJECT_ID('employee_training','U') IS NULL CREATE TABLE employee_training(id BIGINT IDENTITY PRIMARY KEY,venue_id BIGINT NOT NULL REFERENCES venues(id),employee_name NVARCHAR(250) NOT NULL,job_title NVARCHAR(250),training_date DATE,allergens_in_house BIT NOT NULL DEFAULT 0,allergens_course BIT NOT NULL DEFAULT 0,food_hygiene_2 BIT NOT NULL DEFAULT 0,food_hygiene_3 BIT NOT NULL DEFAULT 0,fire_alarm BIT NOT NULL DEFAULT 0,fire_evacuation BIT NOT NULL DEFAULT 0,fire_extinguisher BIT NOT NULL DEFAULT 0,fire_exits BIT NOT NULL DEFAULT 0,emergency_procedures BIT NOT NULL DEFAULT 0,notes NVARCHAR(MAX),created_at DATETIME2 DEFAULT SYSUTCDATETIME(),created_by BIGINT,updated_at DATETIME2 DEFAULT SYSUTCDATETIME(),updated_by BIGINT); IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE name='idx_employee_training_venue') CREATE INDEX idx_employee_training_venue ON employee_training(venue_id,employee_name);`,
  },
];
