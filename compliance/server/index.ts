...
app.get("/api/risk-assessments/dashboard", async (req: AuthedRequest, res) => {
  ...
  res.json({ current: assessments.filter((a) => a.status === "Current").length, draft: assessments.filter((a) => a.status === "Draft" && !a.site_verification_required).length, reviewDue: assessments.filter((a) => a.status === "Review Due" || (a.review_date && a.review_date <= today)).length, overdue: assessments.filter((a) => a.review_date && a.review_date < today && a.status !== "Archived").length, actionRequired: assessments.filter((a) => a.status === "Action Required").length, openFireActions, highRisk: high.length, siteVerification: assessments.filter((a) => a.site_verification_required).length });
});
