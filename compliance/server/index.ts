...
app.get("/api/risk-assessments/dashboard", async (req: AuthedRequest, res) => {
  ...
  const assessments = await rows<any>(`SELECT * FROM risk_assessments${where}`, ...params), today = new Date().toISOString().slice(0, 10), activeAssessments = assessments.filter((a) => a.status !== "Archived");
  ...
  res.json({ contentReviewed: activeAssessments.filter((a) => a.content_reviewed_at).length, current: activeAssessments.filter((a) => a.status === "Current").length, draft: activeAssessments.filter((a) => a.status === "Draft" && !a.site_verification_required).length, reviewDue: activeAssessments.filter((a) => a.status === "Review Due" || (a.review_date && a.review_date <= today)).length, overdue: activeAssessments.filter((a) => a.review_date && a.review_date < today).length, actionRequired: activeAssessments.filter((a) => a.status === "Action Required").length, openFireActions, highRisk: high.length, siteVerification: activeAssessments.filter((a) => a.site_verification_required).length, nextReviewDate });
});
