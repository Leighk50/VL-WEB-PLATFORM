...
const trainingSchema=z.object({venue_id:z.coerce.number().int().positive(),employee_name:z.string().trim().min(1).max(250),job_title:z.string().trim().max(250).optional().nullable(),training_date:z.string().date().optional().nullable(),allergens_in_house:z.coerce.number().int().min(0).max(1),allergens_course:z.coerce.number().int().min(0).max(1),food_hygiene_2:z.coerce.number().int().min(0).max(1),food_hygiene_3:z.coerce.number().int().min(0).max(1),fire_alarm:z.coerce.number().int().min(0).max(1),fire_evacuation:z.coerce.number().int().min(0).max(1),fire_extinguisher:z.coerce.number().int().min(0).max(1),fire_exits:z.coerce.number().int().min(0).max(1),emergency_procedures:z.coerce.number().int().min(0).max(1),notes:z.string().trim().max(5000).optional().nullable()}).strict();
app.get("/api/training-log",async(req:AuthedRequest,res)=>{const params=isAdmin(req)?[]:[req.user!.venueId];res.json(await rows(`SELECT t.*,v.name venue_name FROM employee_training t JOIN venues v ON v.id=t.venue_id${params.length?" WHERE t.venue_id=?":""} ORDER BY t.employee_name,t.id`,...params));});
app.post("/api/training-log",canWrite,async(req:AuthedRequest,res)=>{const p=trainingSchema.safeParse(req.body);if(!p.success)return res.status(400).json({error:"Invalid training record",issues:p.error.flatten()});if(!await assertVenue(req,res,p.data.venue_id))return;const keys=Object.keys(p.data),r=await db.run(`INSERT INTO employee_training(${keys.join(",")},created_by) VALUES(${keys.map(()=>"?").join(",")},?)`,[...Object.values(p.data),req.user!.id]),after=await db.get("SELECT * FROM employee_training WHERE id=?",[r.lastInsertRowid]);await audit("employee_training",r.lastInsertRowid,"create",null,after,req.user!.id,req.ip);res.status(201).json(after);});
app.patch("/api/training-log/:id",canWrite,async(req:AuthedRequest,res)=>{const id=Number(req.params.id),before=await db.get<any>("SELECT * FROM employee_training WHERE id=?",[id]);if(!before)return res.status(404).json({error:"Training record not found"});if(!venueAllowed(req,Number(before.venue_id)))return res.status(403).json({error:"Venue access denied"});const p=trainingSchema.partial().safeParse(req.body);if(!p.success||!Object.keys(p.data).length)return res.status(400).json({error:"Invalid training update",issues:p.success?undefined:p.error.flatten()});if(p.data.venue_id&&!await assertVenue(req,res,p.data.venue_id))return;const keys=Object.keys(p.data);await db.run(`UPDATE employee_training SET ${keys.map(k=>`${k}=?`).join(",")},updated_at=CURRENT_TIMESTAMP,updated_by=? WHERE id=?`,[...Object.values(p.data),req.user!.id,id]);const after=await db.get("SELECT * FROM employee_training WHERE id=?",[id]);await audit("employee_training",id,"update",before,after,req.user!.id,req.ip);res.json(after);});
...
app.get("/api/risk-assessments", async (req: AuthedRequest, res) => {
  ...
  res.json(await rows(`SELECT r.*,v.name venue_name,l.name location_name,(SELECT count(*) FROM risk_hazards h WHERE h.assessment_id=r.id) hazard_count,(SELECT count(*) FROM risk_hazards h WHERE h.assessment_id=r.id AND h.further_action IS NOT NULL AND h.further_action<>'') unresolved_count,(SELECT count(*) FROM risk_hazards h WHERE h.assessment_id=r.id AND h.residual_score>=10) high_risk_count,(SELECT count(*) FROM actions a WHERE a.status NOT IN ('Closed','Complete') AND ((a.related_type='risk_assessment' AND a.related_id=r.id) OR (a.related_type='risk_assessment_hazard' AND a.related_id IN (SELECT id FROM risk_hazards WHERE assessment_id=r.id)))) open_action_count,(SELECT count(*) FROM document_links dl WHERE dl.entity_type='risk_assessment' AND dl.entity_id=r.id) document_count,(SELECT count(*) FROM photos p WHERE p.entity_type='risk_assessment' AND p.entity_id=r.id) photo_count FROM risk_assessments r JOIN venues v ON v.id=r.venue_id LEFT JOIN locations l ON l.id=r.location_id${scope.sql} ORDER BY CASE r.status WHEN 'Action Required' THEN 0 WHEN 'Review Due' THEN 1 WHEN 'Requires Site Verification' THEN 2 WHEN 'Draft' THEN 3 ELSE 4 END,r.title`, ...scope.params));
});
app.get("/api/risk-assessments/dashboard", async (req: AuthedRequest, res) => {
  ...
  const assessments = await rows<any>(`SELECT * FROM risk_assessments${where}`, ...params), today = new Date().toISOString().slice(0, 10), soon = new Date(Date.now() + 30 * 864e5).toISOString().slice(0, 10), activeAssessments = assessments.filter((a) => a.status !== "Archived");
  const ids = assessments.map((item) => Number(item.id));
  const high = ids.length ? await rows<any>(`SELECT h.*,r.title,r.area FROM risk_hazards h JOIN risk_assessments r ON r.id=h.assessment_id WHERE h.assessment_id IN (${ids.map(() => "?").join(",")}) AND h.residual_score>=10 AND r.status<>'Archived'`, ...ids) : [];
  ...
  const nextReviewDate = activeAssessments.map((a) => a.review_date).filter(Boolean).sort()[0] || null;
  const openRiskActions = ids.length ? await rows<any>(`SELECT DISTINCT CASE WHEN a.related_type='risk_assessment' THEN a.related_id ELSE h.assessment_id END assessment_id FROM actions a LEFT JOIN risk_hazards h ON a.related_type='risk_assessment_hazard' AND h.id=a.related_id WHERE a.status NOT IN ('Closed','Complete') AND ((a.related_type='risk_assessment' AND a.related_id IN (${ids.map(() => "?").join(",")})) OR (a.related_type='risk_assessment_hazard' AND h.assessment_id IN (${ids.map(() => "?").join(",")})))`, ...ids, ...ids) : [];
  const actionAssessmentIds = new Set(openRiskActions.map((row) => Number(row.assessment_id)));
  res.json({ contentReviewed: activeAssessments.filter((a) => a.content_reviewed_at).length, current: activeAssessments.filter((a) => a.status === "Current").length, draft: activeAssessments.filter((a) => a.status === "Draft" && !a.site_verification_required).length, reviewDue: activeAssessments.filter((a) => a.status === "Review Due" || (a.review_date && a.review_date >= today && a.review_date <= soon)).length, overdue: activeAssessments.filter((a) => a.review_date && a.review_date < today).length, actionRequired: activeAssessments.filter((a) => a.status === "Action Required" || actionAssessmentIds.has(Number(a.id))).length, openFireActions, highRisk: new Set(high.map((row) => Number(row.assessment_id))).size, siteVerification: activeAssessments.filter((a) => a.site_verification_required).length, nextReviewDate });
});
...
const riskActionBody = z.object({
  description: z.string().trim().max(10000).optional(),
  responsible_person: z.string().trim().max(500).optional(),
...
app.post("/api/risk-hazards/:id/action", canWrite, async (req: AuthedRequest, res) => {
  ...
  const parsed = riskActionBody.safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: "Invalid Action / Defect", issues: parsed.error.flatten() });
...
app.post("/api/risk-assessments/:id/review", canWrite, async (req: AuthedRequest, res) => {
  ...
  try {
    await riskSnapshot(old, "Version closed for review", req.user!.id); const cols = ["venue_id","title","category","area","location_id","assessment_date","assessor","responsible_person","review_date","status","overall_risk_rating","notes","site_verification_required","version","previous_version_id","template_key","signed_by","signed_at","signoff_notes","content_reviewed_at","content_review_note"];
  ...
  app.get(`/api/${route}`, async (req: AuthedRequest, res) => {
    ...
    const attachmentColumn =
      route === "documents"
        ...
        : "";
    const drilldownColumns = route === "assets"
      ? ",(SELECT max(p.next_date) FROM pat_tests p WHERE p.asset_id=t.id) pat_next_date"
      : route === "actions"
        ? ",CASE WHEN t.related_type='risk_assessment' THEN (SELECT title FROM risk_assessments WHERE id=t.related_id) WHEN t.related_type='risk_assessment_hazard' THEN (SELECT r.title FROM risk_hazards h JOIN risk_assessments r ON r.id=h.assessment_id WHERE h.id=t.related_id) WHEN t.related_type='asset' THEN (SELECT description FROM assets WHERE id=t.related_id) WHEN t.related_type='pat_test' THEN (SELECT a.description FROM pat_tests p JOIN assets a ON a.id=p.asset_id WHERE p.id=t.related_id) ELSE NULL END source_title,CASE WHEN t.related_type='risk_assessment' THEN (SELECT category FROM risk_assessments WHERE id=t.related_id) WHEN t.related_type='risk_assessment_hazard' THEN (SELECT r.category FROM risk_hazards h JOIN risk_assessments r ON r.id=h.assessment_id WHERE h.id=t.related_id) WHEN t.related_type LIKE 'food_%' THEN 'food' ELSE NULL END source_category,CASE WHEN t.related_type IN ('risk_assessment','asset') THEN t.related_id WHEN t.related_type='risk_assessment_hazard' THEN (SELECT assessment_id FROM risk_hazards WHERE id=t.related_id) WHEN t.related_type='pat_test' THEN (SELECT asset_id FROM pat_tests WHERE id=t.related_id) ELSE NULL END source_record_id"
        : "";
    res.json(
      await rows(
        `SELECT t.*,v.name venue_name${locationColumn}${attachmentColumn}${drilldownColumns} FROM ${cfg.table} t LEFT JOIN venues v ON v.id=t.venue_id${joinLocation}${scoped.sql} ORDER BY t.id DESC`,
        ...scoped.params,
      ),
    );
  });
...
async function createAccessToken(userId:number,purpose:"invitation"|"password_reset",createdBy:number){const token=randomBytes(32).toString("base64url"),hash=createHash("sha256").update(token).digest("hex"),hours=purpose==="invitation"?48:1,expiresAt=new Date(Date.now()+hours*3600_000).toISOString(),setupPath=`/set-password?token=${encodeURIComponent(token)}`,baseUrl=(process.env.APP_BASE_URL||(config.NODE_ENV==="production"?"https://compliance.villagelimits.co.uk":"http://localhost:5173")).replace(/\/$/,"");await db.run("UPDATE user_access_tokens SET used_at=CURRENT_TIMESTAMP WHERE user_id=? AND purpose=? AND used_at IS NULL",[userId,purpose]);await db.run("INSERT INTO user_access_tokens(user_id,purpose,token_hash,expires_at,created_by) VALUES(?,?,?,?,?)",[userId,purpose,hash,expiresAt,createdBy]);return {setup_path:setupPath,setup_url:`${baseUrl}${setupPath}`,expires_at:expiresAt};}
