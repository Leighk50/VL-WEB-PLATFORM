...
if(p==="/api/admin/test-email"&&req.method==="POST"){
  ...
  if(p==="/api/admin/content"&&req.method==="PUT"){if(!valid(req))return json(res,401,{error:"Your admin session has expired. Please sign in again."});const incoming=await body(req),current=read();const liveSpecials=current.menus?.find(m=>m.id==="specials"),submittedSpecials=incoming.menus?.find(m=>m.id==="specials");if(liveSpecials?.updatedAt&&(!submittedSpecials?.updatedAt||Date.parse(submittedSpecials.updatedAt)<Date.parse(liveSpecials.updatedAt))){if(!Array.isArray(incoming.menus))incoming.menus=[];const index=incoming.menus.findIndex(m=>m.id==="specials");if(index>=0)incoming.menus[index]=liveSpecials;else incoming.menus.push(liveSpecials)}write(incoming);return json(res,200,{ok:true})}
