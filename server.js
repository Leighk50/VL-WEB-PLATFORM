...
const BUILD=process.env.GITHUB_SHA?process.env.GITHUB_SHA.slice(0,7):"local",VERSION="2.2.0",SITE=(process.env.PUBLIC_SITE_URL||"https://www.villagelimits.co.uk").replace(/\/+$/,""),AV=encodeURIComponent(`${VERSION}-${BUILD}`);
...
async function sendChristmasEnquiry(q){
 ...
 if(!r.ok)throw new Error("We could not send your enquiry just now.");
}
