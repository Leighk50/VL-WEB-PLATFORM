...
const STOCK_ROLES = ["component","sold"];
...
function itemRole(item){return item?.stockRole==="sold"?"sold":"component";}
function nullableMoney(value,label){if(value===null||value===undefined||String(value).trim()==="")return null;const n=Number(value);if(!Number.isFinite(n)||n<0)throw new Error(`${label} must be zero or a positive number.`);return Math.round(n*100)/100;}
function consolidateEquivalentItems(data){
  const byKey=new Map();
  const kept=[];
  let changed=false;
  for(const item of data.items||[]){
    if(!STOCK_ROLES.includes(item.stockRole)){item.stockRole="component";changed=true;}
    if(item.sellingPrice===undefined){item.sellingPrice=null;changed=true;}
    ...
    if(itemRole(existing)!=="sold"&&itemRole(item)==="sold")existing.stockRole="sold";
    if((existing.sellingPrice===null||existing.sellingPrice===undefined||existing.sellingPrice==="")&&item.sellingPrice!==null&&item.sellingPrice!==undefined&&item.sellingPrice!=="")existing.sellingPrice=item.sellingPrice;
...
function masterData(data){return {types:TYPES,stockRoles:STOCK_ROLES,items:data.items.map(i=>({...publicItem(i),stockRole:itemRole(i),costPrice:Number.isFinite(Number(i.costPrice))?Number(i.costPrice):null,sellingPrice:Number.isFinite(Number(i.sellingPrice))?Number(i.sellingPrice):null}))};}
function priceListData(data){return {updatedAt:data.updatedAt,items:data.items.filter(i=>itemRole(i)==="sold").map(i=>({...publicItem(i),sellingPrice:Number.isFinite(Number(i.sellingPrice))?Number(i.sellingPrice):null})).sort((a,b)=>a.type.localeCompare(b.type)||a.description.localeCompare(b.description)||a.size.localeCompare(b.size))};}
...
function priceListPage(data){
  const report=priceListData(data);
  const rows=report.items.map(i=>`<tr><td>${esc(i.type)}</td><td><strong>${esc(i.description)}</strong></td><td>${esc(i.size)}</td><td class="num">${i.sellingPrice==null?"—":`£${esc(i.sellingPrice.toFixed(2))}`}</td></tr>`).join("");
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Bar Price List | Village Limits</title><style>@page{size:A4;margin:12mm}body{font-family:Arial,sans-serif;color:#222;margin:0;background:#f4f1eb}.toolbar{padding:14px;text-align:center}.toolbar button{padding:10px 18px;border:0;border-radius:20px;background:#222;color:#fff;font-weight:700}.sheet{max-width:900px;margin:auto;background:#fff;padding:28px 34px;box-shadow:0 4px 18px #0002}.head{text-align:center;border-bottom:2px solid #b69452;padding-bottom:16px;margin-bottom:18px}.head img{max-width:260px;max-height:100px}.head h1{font-family:Georgia,serif;letter-spacing:2px;margin:12px 0 4px}.meta{color:#666;font-size:12px}table{width:100%;border-collapse:collapse}th{text-align:left;border-bottom:2px solid #333;padding:8px 6px}td{padding:7px 6px;border-bottom:1px solid #ddd}.num{text-align:right;font-weight:700}th.num{text-align:right}@media print{body{background:#fff}.toolbar{display:none}.sheet{box-shadow:none;max-width:none;padding:0}}</style></head><body><div class="toolbar"><button onclick="window.print()">Print Bar Price List</button></div><main class="sheet"><div class="head"><img src="/assets/images/logo-gold.png" alt="Village Limits"><h1>BAR PRICE LIST</h1><div class="meta">Items marked as sold in the Master Stock List</div></div><table><thead><tr><th>Type</th><th>Description</th><th>Size</th><th class="num">Selling Price</th></tr></thead><tbody>${rows||'<tr><td colspan="4">No items are currently marked as sold.</td></tr>'}</tbody></table></main></body></html>`;
}
function stockPanelHtml(owner){return `<button data-panel="stocktake">Bar Stock Take</button><button data-panel="stock-report">Total Stock Report</button>${owner?'<button data-panel="master-stock">Master Stock List</button><button data-panel="price-list">Bar Price List</button><button data-panel="stock-access">Stock Access</button>':''}`;}
function stockSectionsHtml(owner){return `<section id="panel-stocktake" class="admin-panel" hidden><div class="admin-heading"><div><span class="eyebrow">Bar</span><h1>Stock Take</h1></div><div class="actions"><button id="refreshStocktake" class="btn dark" type="button">Refresh</button><button id="saveStocktake" class="btn" type="button">Save Stock Take</button></div></div><p id="stockMeta" class="save-status"></p><div class="admin-card"><h3>Find or add a product</h3><p>Start typing a product name. Existing master items are suggested first to prevent duplicates. Equivalent sizes such as 70cl and 700ml are treated as the same size.</p><label>Search master stock<input id="stockSearch" autocomplete="off" placeholder="e.g. Peroni 330ml"></label><div id="stockSuggestions"></div><form id="stockNewItem" style="margin-top:14px"><div class="row-2"><label>Item type<select name="type" required><option value="">Choose type</option></select></label><label>Size<input name="size" required placeholder="330ml, 70cl, 50L keg"></label></div><label>Description<input name="description" required placeholder="Brand / product name"></label><div class="actions"><button id="stockAddBtn" class="btn dark" type="submit">Add New Master Item</button><button id="stockForceAdd" class="btn secondary" type="button" hidden>Add as new anyway</button></div></form></div><div id="stockRows" style="margin-top:18px"></div><p id="stockStatus" class="save-status"></p></section><section id="panel-stock-report" class="admin-panel" hidden><div class="admin-heading"><div><span class="eyebrow">Bar</span><h1>Total Stock Report</h1></div><div class="actions"><button id="refreshStockReport" class="btn dark" type="button">Refresh</button><button id="printStockReport" class="btn" type="button">Print Report</button></div></div><p id="stockReportMeta" class="save-status"></p><div id="stockReportRows"></div></section>${owner?'<section id="panel-master-stock" class="admin-panel" hidden><div class="admin-heading"><div><span class="eyebrow">Bar</span><h1>Master Stock List</h1></div><button id="refreshMasterStock" class="btn dark" type="button">Refresh</button></div><div class="admin-card"><p>Maintain master product details here. Mark an item as <strong>Sold</strong> if it is sold directly to customers, or <strong>Component</strong> if it is only used as part of another product. Cost prices remain hidden from the Stock Take screen.</p><div id="masterStockRows"></div><p id="masterStockStatus" class="save-status"></p></div></section><section id="panel-price-list" class="admin-panel" hidden><div class="admin-heading"><div><span class="eyebrow">Bar</span><h1>Bar Price List</h1></div><div class="actions"><button id="refreshPriceList" class="btn dark" type="button">Refresh</button><button id="printPriceList" class="btn" type="button">Print Price List</button></div></div><p id="priceListMeta" class="save-status"></p><div id="priceListRows"></div></section><section id="panel-stock-access" class="admin-panel" hidden><div class="admin-heading"><div><span class="eyebrow">Security</span><h1>Stock Take Access</h1></div></div><div class="admin-card"><p>Choose which additional admin users can access the Bar Stock Take area. The Owner always has access.</p><div id="stockAccessUsers"></div><button id="saveStockAccess" class="btn dark" type="button">Save Access</button><p id="stockAccessStatus" class="save-status"></p></div></section>':''}`;}
function injectAdmin(req){if(!canStocktake(req))return null;let html=fs.readFileSync(ADMIN_HTML,"utf8");const owner=isOwner(req);html=html.replace('</aside>',`${stockPanelHtml(owner)}</aside>`).replace('</main>',`${stockSectionsHtml(owner)}</main>`).replace('</body>','<script src="/assets/js/stocktake.js?v=1.3.0"></script></body>');return html;}
...
async function handle(req,res,pathname,url){
  ...
  if(pathname==="/admin/stocktake/pricelist"&&req.method==="GET"){
    ...
    res.writeHead(200,{"Content-Type":"text/html; charset=utf-8","Cache-Control":"no-store"});res.end(priceListPage(readStock()));return true;
  }
  ...
  if(pathname==="/api/admin/stocktake/pricelist"&&req.method==="GET"){if(!isOwner(req)){json(res,403,{error:"Only the owner can view the bar price list."});return true;}json(res,200,priceListData(data));return true;}
  ...
  if(pathname==="/api/admin/stocktake/items"&&req.method==="POST"){
    ...
    const now=new Date().toISOString();const item={id:crypto.randomUUID(),type,description,size,stockRole:"component",costPrice:null,sellingPrice:null,createdAt:now,updatedAt:now};data.items.push(item);data.counts[item.id]={bar:0,cellar:0,walk_in_fridge:0};data.updatedAt=now;writeStock(data);json(res,201,{ok:true,item:publicItem(item)});return true;
  }
  if(pathname.startsWith("/api/admin/stocktake/items/")&&req.method==="PUT"){
    if(!isOwner(req)){json(res,403,{error:"Only the owner can edit master stock details and prices."});return true;}
    ...
    const body=payload(await readRaw(req));const type=cleanText(body.type),description=cleanText(body.description),size=cleanText(body.size),stockRole=cleanText(body.stockRole||"component").toLowerCase();
    if(!TYPES.includes(type)||!description||!size){json(res,400,{error:"Type, description and size are required."});return true;}if(!STOCK_ROLES.includes(stockRole)){json(res,400,{error:"Choose Sold or Component."});return true;}
    const key=itemKey(description,size);if(data.items.some(i=>i.id!==id&&itemKey(i.description,i.size)===key)){json(res,409,{error:"Another master item already uses that description and equivalent size."});return true;}
    let costPrice=null,sellingPrice=null;try{costPrice=nullableMoney(body.costPrice,"Cost price");sellingPrice=nullableMoney(body.sellingPrice,"Selling price");}catch(err){json(res,400,{error:err.message});return true;}
    Object.assign(item,{type,description,size,stockRole,costPrice,sellingPrice:stockRole==="sold"?sellingPrice:null,updatedAt:new Date().toISOString()});data.updatedAt=item.updatedAt;writeStock(data);json(res,200,{ok:true,item:{...publicItem(item),stockRole:item.stockRole,costPrice:item.costPrice,sellingPrice:item.sellingPrice}});return true;
  }
