...
async function handle(req,res,pathname,url){
  ...
  if(pathname==="/api/admin/stocktake/search"&&req.method==="GET"){const q=String(url.searchParams.get("q")||"");const results=data.items.map(item=>({item,score:scoreItem(item,q)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,8).map(x=>x.item);json(res,200,{results});return true;}
  ...
  if(pathname==="/api/admin/stocktake/items"&&req.method==="POST"){
    ...
    const key=itemKey(description,size);const duplicate=data.items.find(i=>itemKey(i.description,i.size)===key);if(duplicate){json(res,409,{error:"That product is already in the master list.",existing:duplicate});return true;}
    const suggestions=data.items.map(item=>({item,score:scoreItem(item,`${description} ${size}`)})).filter(x=>x.score>=40).sort((a,b)=>b.score-a.score).slice(0,5).map(x=>x.item);if(suggestions.length&&!body.confirmNew){json(res,409,{error:"Similar products already exist. Select one of them, or confirm this is genuinely a new product.",suggestions});return true;}
    const now=new Date().toISOString();const item={id:crypto.randomUUID(),type,description,size,createdAt:now,updatedAt:now};data.items.push(item);data.counts[item.id]={bar:0,cellar:0,walk_in_fridge:0};data.updatedAt=now;writeStock(data);json(res,201,{ok:true,item});return true;
  }
  if(pathname.startsWith("/api/admin/stocktake/items/")&&req.method==="PUT"){
    const id=decodeURIComponent(pathname.slice("/api/admin/stocktake/items/".length));const item=data.items.find(i=>i.id===id);if(!item){json(res,404,{error:"Stock item not found."});return true;}const body=payload(await readRaw(req));const type=cleanText(body.type),description=cleanText(body.description),size=cleanText(body.size);if(!TYPES.includes(type)||!description||!size){json(res,400,{error:"Type, description and size are required."});return true;}const key=itemKey(description,size);if(data.items.some(i=>i.id!==id&&itemKey(i.description,i.size)===key)){json(res,409,{error:"Another master item already uses that description and size."});return true;}Object.assign(item,{type,description,size,updatedAt:new Date().toISOString()});data.updatedAt=item.updatedAt;writeStock(data);json(res,200,{ok:true,item});return true;
  }
