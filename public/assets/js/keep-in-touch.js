(()=>{
  ...
  form.addEventListener('submit',async e=>{
    ...
    const fd=new FormData(form);const body={name:String(fd.get('name')||'').trim(),email:String(fd.get('email')||'').trim(),mobile:String(fd.get('mobile')||'').trim(),website:String(fd.get('website')||''),marketingConsent:document.getElementById('kitConsent').checked};
    ...
    try{
      ...
      form.reset();message.className='kit-success';message.innerHTML='<strong>Thank you.</strong> You’re on the Village Limits list. We’ll keep you updated with offers and what’s happening.';
