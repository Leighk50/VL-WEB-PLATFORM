(()=>{
  ...
  const prettyAudience=v=>v==='local'?'Local':v==='lincolnshire'?'Lincolnshire':'Visitor / further afield';
  const prettyInterest=v=>v==='restaurant'?'Restaurant':v==='entertainment'?'Entertainment':v==='accommodation'?'Accommodation':v;
  ...
  function render(j){
    ...
    rows.innerHTML=(j.entries||[]).length?`<div class="admin-card" style="overflow:auto"><table style="width:100%;border-collapse:collapse;min-width:1180px"><thead><tr><th style="text-align:left">Name</th><th style="text-align:left">Email</th><th style="text-align:left">Mobile</th><th style="text-align:left">Postcode</th><th style="text-align:left">Audience</th><th style="text-align:left">Interests</th><th style="text-align:left">Consent date</th><th style="text-align:left">Status</th></tr></thead><tbody>${j.entries.map(e=>`<tr style="border-top:1px solid #ddd"><td>${esc(e.name)}</td><td><a href="mailto:${esc(e.email)}">${esc(e.email)}</a></td><td>${esc(e.mobile)}</td><td>${esc(e.postcode||'—')}</td><td>${esc(e.audience?prettyAudience(e.audience):'—')}</td><td>${esc((e.interests||[]).map(prettyInterest).join(', ')||'—')}</td><td>${esc(e.consentAt?new Date(e.consentAt).toLocaleString():'')}</td><td>${e.unsubscribedAt?'<strong style="color:#a33">Unsubscribed</strong>':'<strong style="color:#386b42">Active</strong>'}</td></tr>`).join('')}</tbody></table></div>`:'<div class="admin-card"><p>No registrations yet.</p></div>';
