(()=>{
  ...
  function itemHtml(item,si,ii){const warnings=(item.warnings||[]);return `<div class="admin-card" data-special-item="${si}:${ii}" style="margin:12px 0">
    <div class="row-2"><label>Dish name<input data-field="name" value="${esc(item.name)}"></label><label>Price<input data-field="price" value="${esc(item.price)}" placeholder="£12"></label></div>
    <label>Description<textarea data-field="description">${esc(item.description)}</textarea></label>
    <label>Allergens<input data-field="allergens" value="${esc(item.allergens)}" placeholder="Milk, Crustaceans, Cereals containing gluten"></label>
    ${warnings.length?`<div class="form-error" data-warning-box><strong>Needs confirmation</strong><ul>${warnings.map(w=>`<li>${esc(w)}</li>`).join('')}</ul><button class="small-btn" type="button" data-confirm-warnings>Confirmed / reviewed</button></div>`:''}
  </div>`}
  function render(){
    ...
    editor.innerHTML=`<div class="admin-card"><p><strong>Status:</strong> ${esc(draft.status||'draft')} &nbsp; <strong>Parser:</strong> ${esc(draft.parser||'rules')} &nbsp; <strong>Received:</strong> ${esc(draft.receivedAt?new Date(draft.receivedAt).toLocaleString():'')}</p></div>`+(draft.sections||[]).map((s,si)=>`<section class="admin-card" data-special-section="${si}" style="margin-top:16px"><label>Section name<input data-section-name value="${esc(s.name)}"></label>${(s.items||[]).map((i,ii)=>itemHtml(i,si,ii)).join('')}<button class="small-btn" type="button" data-add-item="${si}">Add dish</button></section>`).join('');
  }
  function collect(){
    ...
    editor.querySelectorAll('[data-special-section]').forEach(sectionEl=>{
      ...
      sectionEl.querySelectorAll('[data-special-item]').forEach(itemEl=>{
        const [si,ii]=itemEl.dataset.specialItem.split(':').map(Number),old=draft.sections?.[si]?.items?.[ii]||{};
  ...
  async function refresh(){try{setStatus('Loading…');const data=await api('/api/admin/specials-sms/status');draft=data.draft||null;metaEl.innerHTML=`<strong>Chef SMS numbers:</strong> ${data.chefNumbers?.length?data.chefNumbers.map(esc).join(', '):'<span style="color:#9b1c1c">Not configured</span>'} &nbsp; <strong>AI parser:</strong> ${data.aiConfigured?'Configured':'Rules fallback only'} &nbsp; <strong>Webhook protection:</strong> ${data.webhookSecretConfigured?'Configured':'Not configured'}`;render();setStatus('')}catch(e){setStatus(e.message,true)}}
