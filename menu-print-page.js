...
function printMenuPage(menu) {
  const sections = (menu.sections || []).map(section => {
    ...
    return `<section class="menu-section"><div class="section-title"><span></span><h2>${esc(section.name)}</h2><span></span></div>${items.map(item => `<article class="menu-item"><div class="dish-row"><h3>${esc(menu.id === "specials" ? formatSpecialsTitle(item.name) : item.name)}</h3><div class="dots"></div><strong>${esc(item.price)}</strong></div>${item.description ? `<p class="description">${esc(item.description)}</p>` : ""}${item.allergens ? `<p class="allergens"><span>Allergens</span> ${esc(item.allergens)}</p>` : ""}</article>`).join("")}</section>`;
  }).join("");
